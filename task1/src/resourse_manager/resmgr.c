...
static void *client_thread(void *arg);  // ИСПРАВЛЕНО: добавлена точка с запятой
...
static void *client_thread(void *arg)
{
  ...
  for (;;) {
    ...
    if (strncmp(buf, "READ", 4) == 0) {
        ...
        const char *data = buf + 6; // Пропускаем "WRITE "
  ...
  }  // ИСПРАВЛЕНО: добавлена закрывающая скобка для for(;;)
