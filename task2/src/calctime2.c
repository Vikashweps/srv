...
int main(void) {
    ...
    for (samples = 0; samples < NUM_SAMPLES; ++samples) {
        ...
        /*TIMER_ABSTIME предотвращает накопление ошибки, потому что
        Каждый вызов clock_nanosleep() включает время на вычисления и
        планирование между получением текущего времени и установкой таймера
        Без TIMER_ABSTIME - ошибка накапливается на каждой итерации */
        int rc;
        do {
        ...
        //сщбираем статистику реальных интервалов между пробуждениями
    ...
    // Расчет стандартного отклонения (по формуле ско)
