 ...
 *
 * Без оптимизаций (SCHED_OTHER):
 *   min: ~5000 ns, avg: ~20000 ns, max: >500000 ns (из-за page faults, миграции, вытеснения)
 *
 * С оптимизациями (SCHED_FIFO + mlockall + CPU affinity):
 *   min: ~1000 ns, avg: ~2000 ns, max: <10000 ns
 *
 * Объяснение:
 * - SCHED_FIFO: исключает вытеснение задачами с низким приоритетом → уменьшает max latency.
 * - mlockall: предотвращает page faults → устраняет задержки от диска/swap.
 * - CPU affinity: избегает миграции между ядрами → сохраняет кэш и TLB, снижает jitter.
 *
 * В совокупности эти техники делают поведение системы предсказуемым,
 * что критично для soft real-time приложений.
 */
