...
#endif
/*clock_nanosleep — простой блокирующий сон, подходит для однопоточных периодических задач.
timerfd — представляет таймер как файловый дескриптор, поэтому предпочтительнее в event-driven приложениях, 
где нужно одновременно обрабатывать таймеры, сокеты и другие события через epoll/poll/select без потоков и сигналов.*/
