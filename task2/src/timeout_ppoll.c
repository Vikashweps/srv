 ...
 *
 * Сценарий:
 * 1. Основной поток блокирует сигнал SIGUSR1 с помощью sigprocmask().
...
static void *thread_sender(void *arg) {
    ...
    // Используем getpid() для отправки сигнала процессу
    kill(getpid(), SIGUSR1);
    return NULL;
}
...
int main(void) {
    ...
    sa.sa_flags = 0; // Явно устанавливаем флаги
    sigemptyset(&sa.sa_mask); // Очищаем маску сигналов в обработчике
    ...
    // 2. Блокируем SIGUSR1 с помощью sigprocmask (более стандартный подход)
...
/*
 * СРАВНЕНИЕ МЕХАНИЗМОВ ТАЙМАУТОВ:
 * 
 * poll() - миллисекунды, множественные fd, уязвим к сигналам
 *    Сети: серверы, мониторинг сокетов
 * 
 * ppoll() - наносекунды, атомарная работа с сигналами  
 *    Сети: надежные приложения, реальное время
 * 
 * pthread_cond_timedwait() - абсолютное время, требует мьютекс
 *    Многопоточность: синхронизация, producer-consumer
 * 
 * mq_timedreceive/send() - абсолютное время, IPC
 *    Межпроцессное: очереди сообщений, распределенные системы
 * 
 * ВЫБОР:
 * - fd: poll/ppoll
 * - потоки: condvar  
